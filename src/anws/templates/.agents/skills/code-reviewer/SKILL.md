...
- **优先路径**：若宿主提供可委派的 **Agent / Task / 子代理** 等工具（以下统称 **AGENT 工具**），**必须**通过 AGENT 专职执行本 skill（向 AGENT 交代完整 skill 约束、必读输入与输出结构），以隔离实现细节与编排上下文。编排侧负责准备输入、触发 AGENT、**将 AGENT 产出的审查正文原样落入** `/forge` 规定的落盘路径（或由 AGENT 直接写入该路径）；**产出格式、Lens、证据规则仍以本文件为准**，AGENT 不得自行删减。
- **回退路径**：仅当宿主**没有**可用的 AGENT 委派能力时，由**当前会话**按本 skill **完整**执行 Lens 1–6，按「输出结构（精简）」六段输出审查全文。
- **禁止借口**：**不得**在已有 AGENT 工具时改由当前会话「顺手代跑」以省步骤。**不得**以「上下文不够」「改动不大」「时间紧」等理由降低证据要求、跳过 Lens 或跳过执行。无 AGENT 能力时由当前会话执行是正常回退，不属敷衍。`/forge` 中的豁免**只能**由用户在波次签名时明示。
