---
description: "【ALPHA】探测系统风险、隐藏耦合与架构暗坑（双语友好中文主述）。适用于接手遗留、重大变更前/后风险评估。产出 `.anws/v{N}/00_PROBE_REPORT.md`（系统指纹、构建/运行时拓扑、Git 热点、风险矩阵）。"
---
...
>
> **探测报告书写契约**：  
> - **精确**：区分「skill 直接输出」「自洽推断」「待用户确认假设」三类，不得混写为一类语气。  
> - **有据**：关键句可指回具体命令、`.nexus-map/` 文件名或 inspector 小节标题。  
> - **不重复**：System Fingerprint 与 Topology 各部分不互相整段拷贝；总则引用节内细节。  
> - **禁止泛泛填充**：禁止无对象的「可能存在风险」「建议优化」而无对应模块路径、严重度或与矩阵行对齐。  
>
