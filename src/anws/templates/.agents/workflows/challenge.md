...
> **后续工作流路由（对用户的下一步建议）**：在建议进入编码链之前，**先检查** `{TARGET_DIR}/05A_TASKS.md` 是否已存在且含有可作为 `/forge` 输入的**真实任务分解**（通常由 `/blueprint` 生成）。**若尚无任务清单、文件不存在或仍为占位/空**（常见：刚完成 `/genesis` 或设计讨论、尚未跑 `/blueprint`）→ **应推荐 `/blueprint`**，**不要**默认推荐 `/forge`。仅在任务清单已就绪且通过上文 Critical/High 门禁时，再推荐 `/forge`。
